...
test.describe('Checkout Process', () => {
  test('Complete Checkout Process with Valid Details', async ({ page }) => {
    ...
    await page.locator('input[placeholder="E-Mail"]').fill('john.fake@example.com');
    ...
    await page.locator('select[name="country_id"]').selectOption({ label: 'US' });
    await page.locator('select[name="zone_id"]').selectOption({ label: 'New York' });
    await page.locator('text=I have read and agree to the Terms & Conditions').check();
    await page.locator('role=button[name="Confirm Order"]').click();
    const successMessage = page.locator('.alert.alert-success.alert-dismissible');
